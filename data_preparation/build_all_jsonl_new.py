...
def build_vipseg():
    ...
    videos_dir = VIPSEG_PATH / "VIPSeg" / "imgs"       # 帧图片目录（可选）
    ...
    # imgs/ 目录可选（mini 数据集可能不包含原始帧图片）
    has_imgs_dir = videos_dir.exists()
    if not has_imgs_dir:
        print(f"[vipseg] 警告: imgs 目录不存在: {videos_dir}")
        print(f"[vipseg] 将继续处理（frame_path 将设置为占位符）")
    ...
    with open(dst, "w", encoding="utf-8") as g:
        for video_gt_dir in sorted(panoptic_dir.iterdir()):
            ...
            for frame_file in sampled_frames:
                ...
                # 设置 frame_path（如果 imgs/ 目录存在，使用实际路径；否则使用占位符）
                if has_imgs_dir:
                    video_frames_dir = videos_dir / video_id
                    frame_img_jpg = video_frames_dir / f"{frame_name}.jpg"
                    frame_img_png = video_frames_dir / f"{frame_name}.png"
                    if frame_img_jpg.exists():
                        frame_path = str(frame_img_jpg.relative_to(VIPSEG_PATH))
                    elif frame_img_png.exists():
                        frame_path = str(frame_img_png.relative_to(VIPSEG_PATH))
                    else:
                        frame_path = f"VIPSeg/imgs/{video_id}/{frame_name}.jpg"  # 占位符
                else:
                    frame_path = f"VIPSeg/imgs/{video_id}/{frame_name}.jpg"  # 占位符
                ...
                rec = {
                    ...
                    "frame_path": frame_path,  # 使用之前设置的变量
