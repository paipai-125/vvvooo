...
**注意**：Student 用 **全参数微调**（不用 LoRA）。理由：VaLR/LT-Tuning/CODI/Coconut/Heima 在 2025-2026 年全部用全参数。LoRA 只有 2024 年的 CCoT 用过，效果不如全参数。

---

## 3. 训练时 latent 生成方式（Coconut/VaLR 原味）
...
GT trajectory 按推理段和感知段交错切分：

```
latent_1 (推理段): [Analyze] I need to determine... <observe type="temporal_locate" target="dog"/>
latent_2 (感知段): <result>Dog appears from 12.5s to 18.3s.</result>
latent_3 (推理段): [Reason] Next I need to see... <observe type="temporal_clip" .../>
...
切分规则不变：`<result>...</result>` 独立成感知段，其余合并为推理段。
...
Phase 2（latent mode，串行 K 步）:
  step 1: input_embed = h_0           → forward → h_1
...
### Step 1: Student latent forward（得到 K 个 hidden）

见 §3 + §5，串行 forward K 步，得到 `h_1, h_2, ..., h_K`。

### Step 2: Translator 还原文字（L_trans）
...
| Translator | 全参数（保留） | 全参数（保留，不变） | 我们需要还原文字做 OPD |
