#!/usr/bin/env bash
# =============================================================================
# 全量训练 —— Teacher_R (推理教师, 标准 CE, 纯文本)
#
# 与 student / teacher_p 完全独立，可以单独 Ctrl-C 终止。
# Teacher_R 不看视频，只看 question + trajectory，速度快。
#
# 用法：
#   bash scripts/train_full_teacher_r.sh
# =============================================================================
set -euo pipefail
cd "$(dirname "$0")/.."
...
WANDB_RUN_NAME="${WANDB_RUN_NAME:-teacher_r-full}"

# 自动检测/安装 wandb
if ! python -c "import wandb" 2>/dev/null; then
    echo "[wandb] 未检测到 wandb，开始 pip install ..."
    pip install --quiet wandb || echo "[wandb][WARN] 安装失败，将继续训练但 loss 不会上传到 wandb"
fi

# ---------- 路径 ----------
DATA_ROOT="$(python -m configs.paths data_root)"
SFT_DIR="${DATA_ROOT}/outputs/stage1_sft"
TRAIN_JSONL="${TRAIN_JSONL:-${SFT_DIR}/stage1_sft_teacher_r.jsonl}"
OUTPUT_DIR="${OUTPUT_DIR:-${DATA_ROOT}/outputs/checkpoints/stage1_sft_teacher_r}"
...
if [[ ! -f "${TRAIN_JSONL}" ]]; then
    echo "[ERROR] Teacher_R 训练数据不存在: ${TRAIN_JSONL}"
    echo "       请先运行: bash scripts/run_sft_full.sh"
    exit 1
fi

echo "============================================================"
echo "[Train Full] role=teacher_r"
